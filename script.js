...
(function () {
  ...
  /* =========================================================
     CURSOR AURA + SOFT CARD TILT (desktop only)
     ---------------------------------------------------------
     The tilt is deliberately gentle. A beauty page should feel
     like paper catching light, not like a 3D toy.
     ========================================================= */
    ...
    $$('[data-tilt]').forEach(function (card) {
      card.addEventListener('mousemove', function (e) {
        ...
        card.style.transform =
          'perspective(1100px) rotateX(' + (-py * 3.2).toFixed(2) + 'deg) rotateY(' +
          (px * 3.2).toFixed(2) + 'deg) translateY(-6px)';
      });
      card.addEventListener('mouseleave', function () { card.style.transform = ''; });
    });

    /* ---------- Hero pointer parallax ----------
       Only elements without their own keyframe animation are moved,
       so the hovering bottle keeps floating undisturbed. */
    var heroGrid  = $('.hero__grid');
    var modelWrap = $('.hero__model-frame');
    var bottleArc = $('.hero__bottle-arch');

    if (heroGrid && (modelWrap || bottleArc)) {
      var tx = 0, ty = 0, mx = 0, my = 0, parallaxOn = false;

      heroGrid.addEventListener('mousemove', function (e) {
        var r = heroGrid.getBoundingClientRect();
        tx = (e.clientX - r.left) / r.width - 0.5;
        ty = (e.clientY - r.top) / r.height - 0.5;
        if (!parallaxOn) { parallaxOn = true; parallaxLoop(); }
      }, { passive: true });

      heroGrid.addEventListener('mouseleave', function () { tx = 0; ty = 0; });
      ...
      function parallaxLoop() {
        ...
        if (bottleArc) {
          bottleArc.style.transform =
            'translate3d(' + (mx * 10).toFixed(2) + 'px,' + (my * 8).toFixed(2) + 'px,0)';
        }
